...
import { formatDate, formatTime, distanceKm } from "@/lib/utils";
 ...
 *
 * If OPENAI_API_KEY is set, we call the model with the same data as context.
 * Otherwise we fall back to intent matching over the same data, so the feature
 * is always demonstrable and never hallucinates.
 */
export async function POST(req: Request) {
  ...
  const reply = await answer(message, user.id);
...
async function answer(message: string, userId: string): Promise<string> {
  ...
  if (q.includes("recommend") || q.includes("low battery") || q.includes("should i charge") || q.includes("charge")) {
